...
void Rectangle::draw() const {
    cout << "Drawing rectangle: "
         << "Dimensions { " << width << ", " << height << " }" << endl;
}

int Rectangle::getWidth() const { return width; }

void Rectangle::setWidth(int width) {
    if (width < 0) throw invalid_argument("Width cannot be less than 0");
...
void Rectangle::setHeight(int height) {
    if (height < 0) throw invalid_argument("Height cannot be less than 0");
...
int Rectangle::getObjectsCount() { return Rectangle::objectsCount; }